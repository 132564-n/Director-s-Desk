...
5. 总导演每轮最多提取三张高影响决策卡，用户确认后写入项目事实；
6. 右侧统一管理待决策事项、已确认事实、提案、正式版本和参考图片资产；
7. 保留故事大纲、分场剧本、导演执行包、时长上限和 ZIP 导出能力。
...
node e2e-decisions.mjs
```

`e2e-chat.mjs` 会创建测试项目，请在隔离的数据目录运行。布局检查只读取现有会话；配置页和决策卡测试使用模拟写接口，不会覆盖你的密钥或项目。

## 决策与项目事实

真实模型完成总导演总结后，系统会额外调用一次总导演模型，将高影响创作分岔提取为最多三张决策卡。卡片同时出现在原讨论和右侧“待我决定”中，支持选择推荐方案、自定义、暂缓或否决。

已确认结果按项目级或单集级保存，并自动加入后续讨论上下文；未选方案和已否决方案作为反向约束，避免 Agent 反复提出。重新修改已确认事实会保留修订历史，并提示哪些现有正式产物需要复核。演示模型不会生成决策卡。
...
支持文本讨论、决策事实、剧本与提示词提案以及参考资产管理，不生成样片。图片目前作为资产及文字描述参与上下文，尚未接入模型视觉识别。自主讨论目前有两轮专家发言与总导演总结，不是自适应无限讨论。决策影响提示采用保守策略：修改事实时列出当前已有正式产物，尚未建立逐字段依赖图。模型接口的自动化测试使用模拟供应商，真实供应商可用性需用自己的密钥在网页验证。
